...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    elseif (strlen($descripcion_genero) > 100) {
    ...
    // Si no hay errores, proceder con la inserción
        ...
        $sql_check_nombre = "SELECT id_genero FROM genero WHERE LOWER(nombre_genero) = LOWER(?)";
        $stmt_check_nombre = $conn->prepare($sql_check_nombre);
        $stmt_check_nombre->bind_param("s", $nombre_genero);
        $stmt_check_nombre->execute();
        $result_check_nombre = $stmt_check_nombre->get_result();
        
        if ($result_check_nombre->num_rows > 0) {
            $error = 'Este género ya existe en la base de datos';
            $stmt_check_nombre->close();
        } else {
            $stmt_check_nombre->close();
            
            // Verificar si la descripción ya existe (opcional)
            $sql_check_desc = "SELECT id_genero FROM genero WHERE LOWER(descripcion) = LOWER(?)";
            $stmt_check_desc = $conn->prepare($sql_check_desc);
            $stmt_check_desc->bind_param("s", $descripcion_genero);
            $stmt_check_desc->execute();
            $result_check_desc = $stmt_check_desc->get_result();
            
            if ($result_check_desc->num_rows > 0) {
                $error = 'Esta descripción ya existe para otro género';
                $stmt_check_desc->close();
            } else {
                $stmt_check_desc->close();
                
                // Insertar el nuevo género
                $sql_insert = "INSERT INTO genero (nombre_genero, descripcion) VALUES (?, ?)";
                $stmt_insert = $conn->prepare($sql_insert);
                
                if ($stmt_insert) {
                    $stmt_insert->bind_param("ss", $nombre_genero, $descripcion_genero);
                    ...
                    if ($stmt_insert->execute()) {
                        ...
                        $error = 'Error al agregar el género: ' . $stmt_insert->error;
                    }
...
$sql_generos = "SELECT nombre_genero, descripcion FROM genero";
...
if ($result_generos->num_rows > 0) {
    while($row = $result_generos->fetch_assoc()) {
        $generos_existentes[] = strtolower(trim($row['nombre_genero']));
        if (!empty($row['descripcion'])) {
            $descripciones_existentes[] = strtolower(trim($row['descripcion']));
        }
    }
}

$generos_json = json_encode($generos_existentes);
$descripciones_json = json_encode($descripciones_existentes);
?>
...
<head>
    ...
    <title>Agregar Nuevo Género - Administración</title>
...
<body>
    ...
    <div class="main-container">
        <div class="form-container">
            <h2 class="form-title">Agregar Nuevo Género</h2>
...
const generosExistentes = <?php echo $generos_json; ?>;
const descripcionesExistentes = <?php echo $descripciones_json; ?>;
...
function validarNombre() {
    ...
    if (generosExistentes.includes(nombreLower)) {
        errorText.textContent = 'Este género ya existe en la base de datos';
        errorDiv.style.display = 'block';
        nombreInput.classList.add('error');
        return false;
    }
...
function validarDescripcion() {
    ...
    // Validar si la descripción ya existe (case-insensitive)
    const descripcionLower = descripcion.toLowerCase();
    if (descripcionesExistentes.includes(descripcionLower)) {
        errorText.textContent = 'Esta descripción ya existe para otro género';
        errorDiv.style.display = 'block';
        descripcionInput.classList.add('error');
        return false;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Si hay mensaje de éxito y redirección está configurada, mostrar cuenta regresiva
